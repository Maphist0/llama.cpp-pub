...
void llama_model_sensenova_u1::load_arch_hparams(llama_model_loader & ml) {
    ...
    ml.get_key("sensenova_u1.full_precision", full_precision, false);
...
llama_model_sensenova_u1::graph::graph(const llama_model_sensenova_u1 & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    const bool bf16 = model.output->type == GGML_TYPE_BF16 && !model.full_precision;
    ...
    ggml_tensor * pos[3]{};
    ggml_tensor * inp_pos = build_inp_pos(false);
    ...
    if (model.full_precision) {
        for (int i = 0; i < ggml_graph_n_nodes(gf); ++i) {
            auto * tensor = ggml_graph_node(gf, i);
            if (tensor->op == GGML_OP_MUL_MAT) {
                ggml_mul_mat_set_prec(tensor, GGML_PREC_F32);
            }
        }
    }
}
