...
kernel void kernel_gemm_noshuffle_q4_k_f32_split_k_4x8(
    ...
    const int chunk = ((k + K_SPLITS * 256 - 1) / (K_SPLITS * 256)) * 256;
    ...
    for (int super = k_start; super < k_end; super += QK_K) {
        int sb_idx = super / QK_K;
        ...
        // The eight constant sub-block indices remove packed-scale branches.
        #pragma unroll
        for (int sub_idx = 0; sub_idx < 8; ++sub_idx) {
            int i = super + sub_idx * 32;
...
__attribute__((qcom_reqd_sub_group_size("half")))
kernel void kernel_gemm_noshuffle_q4_k_f32_split_k_2x4(
    global const ushort * src0_q,
    global const uchar  * src0_s,
    global const half   * src0_d,
    global const half   * src0_dm,
    ...
    for (int i = k_start; i < k_end; i += 32) {
        int sb_idx  = i / QK_K;
        int sub_idx = (i / 32) % 8;
        ...
        for (int l = 0; l < 32; l += 4) {
            int ki = i + l;
            ushort2 bits4 = vload2(0, weight_ptr + (ki/4) * m);

            // j=0
            B = read_imageh(src1, gy*1+0 + (ki+0) * n_4);
            dequantized_weights.s0 = (bits4.s0 & 0x000F) * scale.s0 - mval.s0;
            dequantized_weights.s1 = (bits4.s1 & 0x000F) * scale.s1 - mval.s1;
            c0 += B * dequantized_weights.s0;
            c1 += B * dequantized_weights.s1;
            ...
            // j=1
            B = read_imageh(src1, gy*1+0 + (ki+1) * n_4);
            dequantized_weights.s0 = ((bits4.s0 & 0x00F0) >> 4) * scale.s0 - mval.s0;
            dequantized_weights.s1 = ((bits4.s1 & 0x00F0) >> 4) * scale.s1 - mval.s1;
            c0 += B * dequantized_weights.s0;
            c1 += B * dequantized_weights.s1;
            ...
            // j=2
            B = read_imageh(src1, gy*1+0 + (ki+2) * n_4);
            dequantized_weights.s0 = ((bits4.s0 & 0x0F00) >> 8) * scale.s0 - mval.s0;
            dequantized_weights.s1 = ((bits4.s1 & 0x0F00) >> 8) * scale.s1 - mval.s1;
            c0 += B * dequantized_weights.s0;
            c1 += B * dequantized_weights.s1;
            ...
            // j=3
            B = read_imageh(src1, gy*1+0 + (ki+3) * n_4);
            dequantized_weights.s0 = ((bits4.s0 & 0xF000) >> 12) * scale.s0 - mval.s0;
            dequantized_weights.s1 = ((bits4.s1 & 0xF000) >> 12) * scale.s1 - mval.s1;
            c0 += B * dequantized_weights.s0;
            c1 += B * dequantized_weights.s1;
    ...
    int idx = (gy*4)*m + (gx<<1);
    if (gy*4+0 < n_no_padding) {
        vstore2((float2)(c0.s0, c1.s0), 0, dst + idx);
    }
    idx += m;
    if (gy*4+1 < n_no_padding) {
        vstore2((float2)(c0.s1, c1.s1), 0, dst + idx);
    }
    idx += m;
    if (gy*4+2 < n_no_padding) {
        vstore2((float2)(c0.s2, c1.s2), 0, dst + idx);
    }
    idx += m;
    if (gy*4+3 < n_no_padding) {
        vstore2((float2)(c0.s3, c1.s3), 0, dst + idx);
    }
    idx += m;
}

kernel void kernel_reduce_q4_k_split_k(global const float * partials, global float * dst, ulong offsetd, int count) {
    dst = (global float *)((global char *)dst + offsetd);
